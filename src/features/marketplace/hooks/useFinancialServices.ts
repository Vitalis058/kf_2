    ...
    (
      ...
      if (filters.selectedIndustry !== "all") {
        filtered = filtered.filter((service) =>
          service.industry
            .toLowerCase()
            .includes(filters.selectedIndustry.toLowerCase())
        );
      }
      ...
      if (filters.selectedPricing !== "all") {
        filtered = filtered.filter((service) => {
          switch (filters.selectedPricing) {
            case "low-cost":
              return (
                service.cost.includes("2.5%") || service.cost.includes("2.1%")
              );
            case "standard":
              return (
                service.cost.includes("2.8%") || service.cost.includes("3.2%")
              );
            case "premium":
              return (
                service.cost.includes("3.5%") || service.cost.includes("4%")
              );
            case "equity":
      ...
      if (facets.length > 0) {
        Object.keys(filterStates).forEach((facetCode) => {
          const selectedValues = Object.keys(
            (filterStates as Record<string, any>)[facetCode] || {}
          ).filter(
